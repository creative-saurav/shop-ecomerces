...
<div class="page-title px-6 py-6 border-b border-slate-200/80">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-main">Create Brand</h1>
            <p class="text-sm text-slate-500">Add a new brand for your ecommerce store.</p>
        </div>
...
<div class="p-6">
    ...
    <form action="{{ route('admin.brand.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
        @csrf
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4" class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <div>
                ...
                <input id="meta_description" name="meta_description" value="{{ old('meta_description') }}" type="text"
                    class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <label class="inline-flex items-center gap-3 text-sm text-slate-700">
                <input type="checkbox" name="featured" value="1" {{ old('featured') ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                Featured
            </label>
        </div>
