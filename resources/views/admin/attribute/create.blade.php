...
<div class="page-title px-6 py-6 border-b border-slate-200/80">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-main">Create Attribute</h1>
            <p class="text-sm text-slate-500">Add a new attribute for your ecommerce products.</p>
        </div>
...
<div class="p-6">
    ...
    <form action="{{ route('admin.attribute.store') }}" method="POST" class="space-y-6" id="attributeForm">
        @csrf
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <div>
                ...
                <input id="slug" name="slug" value="{{ old('slug') }}" type="text" readonly disabled
                    class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
            </div>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <div>
                ...
                <input id="sort_order" name="sort_order" value="{{ old('sort_order', 0) }}" type="number" min="0"
                    class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <label class="inline-flex items-center gap-3 text-sm text-slate-700">
                <input type="checkbox" name="is_required" value="1" {{ old('is_required') ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                Required
            </label>
        </div>
        ...
        <div>
            ...
            <div id="valuesContainer" class="space-y-3">
                <div class="value-row grid gap-3 lg:grid-cols-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
                    <div>
                        <input type="text" name="values[0][value]" placeholder="Value name" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500" />
                    </div>
                    <div>
                        <input type="text" name="values[0][color_code]" placeholder="Color code #000000" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500" />
                    </div>
                    <div class="flex gap-2">
                        <input type="number" name="values[0][price_adjustment]" placeholder="Price adjust" step="0.01" class="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500" />
